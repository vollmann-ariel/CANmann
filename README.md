...
- "Most Active": ranks signals by how many times their value actually changed — not just how often the message was transmitted — showing each signal's channel and source too, with its own search box (same wildcard/Escape behavior). Right-click a row to jump to that message in "Messages Seen", filter this same list down to only the signals from that message, or copy its message/signal name.
