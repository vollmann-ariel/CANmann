...
- Virtualized table (handles hundreds of thousands of messages without lag), with filtering by ID/name/channel — `*` works as a wildcard, Escape clears it — and mutually-exclusive "decoded only"/"undecoded only" toggles.
...
- Keyboard navigation: ↑/↓ selects the previous/next message, Page Up/Down jumps by a screenful, Home/End jump to the first/last message. Right-click any row for "Filter by message id" or "Jump to time in Plots" (centers the Plots view on that instant and drops a marker there until you click a chart).
- Undecoded messages show their raw bytes colored green/red where a byte rose/fell versus the previous message with the same id, SavvyCAN-style.
